...
def recommend_movies(genre= None, mood = None, rating= None, top_n=5):
    flitered_df = movies_df
    if genre:
        flitered_df = flitered_df[flitered_df['Genre'].str.contains(genre, case = False, na = False)]
    if rating:
        flitered_df= flitered_df[flitered_df['IMDB_Rating'] >= rating]

    flitered_df = flitered_df.sample(frac=1).reset_index(drop=True)

    recommendations = []

    for idx, row in flitered_df.iterrows():
...
def handle_ai(name):
    ...
    print(Fore.GREEN + "Availabe Genres: ", end="")
    ...
    recs = recommend_movies(genre= None, mood = None, rating= None, top_n=5)
